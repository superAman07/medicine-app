...
export async function POST(req: NextRequest) {
  try {
    const { updatedData,medicineName, stockQuantity, salePrice, purchasePrice, expiryDate } = await req.json();
    ...
    for (const sheetName in updatedData) {
      if (Array.isArray(updatedData[sheetName])) { 
        ...
        if(sheetName.toLowerCase()=== 'stock'){
          sheetData.push({Medicine_Name: medicineName, Stock_Quantity: stockQuantity,Purchase_Price: purchasePrice,Sale_Price: salePrice,Expiry_Date: expiryDate})
